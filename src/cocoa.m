...
- (void) windowDidResize:(NSNotification *)n {
        ...
        NSSize sz = fr.size;
        gsInject(GSC_RESIZE, sz.width, sz.height);
...
- (void) keyUp: (NSEvent *)ev {
        [self handleKeyEvent: ev mode: GSC_UP];
}

- (void) keyDown: (NSEvent *)ev {
        [self handleKeyEvent: ev mode: GSC_DOWN];
        [self interpretKeyEvents: [NSArray arrayWithObject: ev]];
}

- (void)deleteBackward:(id)sender {
        gsInject(GSC_UNICODE, GSK_DELETE, 0);
}
...
- (void)insertText:(id)s {
        ...
        int i;
        for (i = 0; i < sl; i++)
                gsInject(GSC_UNICODE, [s characterAtIndex: i], 0);
}
...
- (void) handleMod: (unsigned)mask 
        ...
        if (mask & pressed)
                gsInject(GSC_DOWN, [self keyFor: mask], 0);
        if (mask & released)
                gsInject(GSC_UP, [self keyFor: mask], 0);
}
...
- (void) mouseDown: (NSEvent*)ev {
        [self handleMouseEvent: ev mode: GSC_DOWN];
}

- (void) mouseUp: (NSEvent*)ev {
        [self handleMouseEvent: ev mode: GSC_UP];
}
...
- (void)scrollWheel:(NSEvent *)ev {
        ...
        gsInject(GSC_DOWN, k, 0);
        gsInject(GSC_UP, k, 0);
}

- (void) handleMotion: (NSEvent *)ev {
        NSPoint l = [ev locationInWindow];
        gsInject(GSC_MOTION, l.x, [self frame].size.height - l.y);
}
...
int main(int argc, char ** argv) {
	...
	do {
		...
		NSEvent * ev = [win nextEventMatchingMask: NSAnyEventMask 
