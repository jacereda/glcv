...
static void handle(Display * dpy, Window win, XIC xic, XEvent * e) {
		...
		gsInject(GSC_GLINIT, 0, 0);
                break;
        case ButtonPress:
                gsInject(GSC_DOWN, mapButton(e->xbutton.button), 0);
                break;
        case ButtonRelease:
                gsInject(GSC_UP, mapButton(e->xbutton.button), 0);
                break;
        case MotionNotify:
                gsInject(GSC_MOTION, e->xmotion.x, e->xmotion.y);
                break;
        case KeyPress:
        {
                wchar_t buf[64];
                ...
                int i;
                int n;
                gsInject(GSC_DOWN, mapkeycode(e->xkey.keycode), 0);
                ...
                if (st == XLookupChars || st == XLookupBoth) 
                        for (i = 0; i < n; i++)
                                gsInject(GSC_UNICODE, buf[i], 0);
        }
        break;
        case KeyRelease:
                if (!isAutoRepeat(dpy, win, e)) 
                        gsInject(GSC_UP, mapkeycode(e->xkey.keycode), 0);
